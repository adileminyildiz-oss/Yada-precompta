...
- **`yada-addon38`** : **Connexion double portail** — identifiant (e-mail) + mot de passe ; espace auto-détecté (Cabinet / Client) selon les identifiants ; empreintes SHA-256 salées de (email+mdp) embarquées (`CAB`/`CLI`, clair absent du source) ; verrouillage auto par inactivité + déconnexion ; carte « Connexion & sécurité » dans Paramétrage. `secEssayer/secVerrouiller`, session via `sessionStorage['yada-role']`. Sécurité côté navigateur (dissuasive). Identifiants changés = redéploiement.
