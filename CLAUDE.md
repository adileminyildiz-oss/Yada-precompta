...
- **`yada-addon38`** : **Connexion en 2 temps avant le choix des dossiers** — (1) écran de **choix de l'espace** (Cabinet / Client), (2) **identifiant (e-mail) + mot de passe** propres à l'espace ; empreintes SHA-256 salées de (email+mdp) embarquées (`CAB`/`CLI`, clair absent du source) ; cloisonnement des espaces ; verrouillage auto par inactivité + déconnexion ; session via `sessionStorage['yada-role']` ; carte « Connexion & sécurité » (Paramétrage). `secChoisir/secEssayer/secRetour/secVerrouiller`.
